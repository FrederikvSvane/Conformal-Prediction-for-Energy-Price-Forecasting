...
ARIMAX models decompose the forecast into three components, specified by the order $(p, d, q)$.

The AutoRegressive (AR) component captures how the target depends linearly on its own past values. With order $p$, the model uses the previous $p$ observations:
\begin{equation*}
    y_t = c + \sum_{i=1}^{p} \phi_i y_{t-i} + \epsilon_t
\end{equation*}
where $\phi_i$ are the AR coefficients and $c$ is a constant.

The Integrated (I) component with order $d$ applies differencing $d$ times to make the series stationary. For $d=1$, we model $\Delta y_t = y_t - y_{t-1}$ instead of $y_t$ directly. This removes trends that would otherwise violate stationarity assumptions.

The Moving Average (MA) component models the target as a function of past forecast errors. With order $q$, it incorporates the previous $q$ error terms:
\begin{equation*}
    y_t = \mu + \sum_{j=1}^{q} \theta_j \epsilon_{t-j} + \epsilon_t
\end{equation*}
where $\theta_j$ are the MA coefficients, $\mu$ is the mean, and $\epsilon_t$ represents white noise.

Combining all three components with the exogenous variables $X$ (wind forecasts, temperature, consumption, etc.), the full ARIMAX model becomes:
\begin{equation*}
...
Once the optimal order is identified, we fit the final model using \texttt{statsmodels.SARIMAX}. We set avoid seasonality, since seasonal ARIMA requires multiple complete seasonal cycles to reliably estimate seasonal parameters. With only one year of data (Feb 2022 - Feb 2023), we cannot observe repeated annual patterns, making seasonal parameter estimation unreliable.
...
The dataset is split 80/20 along the temporal axis. Training data spans February 1 to November 20, 2022 (7,026 observations), and test data spans November 20, 2022 to February 1, 2023 (1,757 observations). This split ensures the test period includes the volatile winter months of the energy crisis, providing a test of how well the conformal methods handle distribution shifts.
...
\begin{figure}[H]
    ...
    \caption{SARIMAX results with Confidence Intervals for both OGD and ECI}
    \label{graph:LSTM}
\end{figure}

Figure~\ref{graph:LSTM} presents the SARIMAX model results with similar layout. Compared to LSTM, both conformal methods produce noticeably wider intervals throughout the test period. The predicted prices (orange dashed line) show larger deviations from actual prices, and the intervals remain persistently wide even during stable periods, reflecting the classical model's higher prediction uncertainty.
...
\begin{figure}[H]
    ...
    \label{graph:LSTM_pred_vs_actual_CI}
\end{figure}

Figure~\ref{graph:LSTM_pred_vs_actual_CI} shows the LSTM model predictions with conformal intervals over the test period. The top panel displays OGD intervals (purple shaded region), while the bottom panel shows ECI intervals (green shaded region). Both methods track the actual prices (blue line) well, with intervals widening during volatile periods (notably around timesteps 400-700) and tightening during stable periods. Visually, the ECI intervals appear narrower, particularly during stable periods between timesteps 800-1200, suggesting more adaptive behavior.
...
Table~\ref{tab:results} quantifies the performance differences observed in the figures. For the LSTM model, OGD achieves 90.2\% coverage with average interval width of 107 DKK/MWh, closely matching the target 90\% coverage rate. The basic ECI variant produces narrower intervals (101 DKK/MWh average and median), but with slightly lower coverage at 88.6\%. ECI-cutoff shows similar performance (88.6\% coverage, 101 DKK/MWh average width), while ECI-integral achieves the narrowest intervals (92 DKK/MWh average) at the cost of significantly undercovering at 82.2\%.

For the ARIMAX model, all methods produce substantially wider intervals. OGD achieves exactly 90.0\% coverage with 222 DKK/MWh average width, while the ECI variants show marginal width reductions (219-220 DKK/MWh) but fall below target coverage (86.0-88.7\%). The difference in interval widths between LSTM and ARIMAX (roughly 2× wider for ARIMAX) confirms the visual observations and reflects the classical model's higher prediction uncertainty.

\subsection{Analysis}
\label{ssec:analysis}

All methods achieve coverage within a reasonable range of the target 90\%. The key differentiator is interval efficiency. For LSTM, basic ECI achieves an average interval width of 101 DKK/MWh compared to OGD's 107 DKK/MWh—a reduction of 6 DKK/MWh, or approximately 5.6\%. This improvement is achieved while maintaining 88.6\% coverage, only 1.6 percentage points below target. By incorporating error magnitude rather than binary feedback, ECI adaptively tightens intervals during stable periods (visible in Figure~\ref{graph:LSTM_pred_vs_actual_CI} around timesteps 800-1200), making it the superior choice when base model quality is high.

The ARIMAX results reveal that interval width is primarily driven by base model quality. ARIMAX ECI shows roughly double the average width (219 DKK/MWh) compared to LSTM ECI (101 DKK/MWh) due to the classical model's larger prediction errors. When base predictions are poor, error-quantification provides minimal advantage over binary feedback—all ARIMAX methods differ by only 3 DKK/MWh. This underscores that conformal methods amplify base model quality rather than compensate for it.

It is worth noting that as the test period progresses, the conformal methods accumulate more observations, allowing the threshold $q_t$ to adapt more effectively. This suggests that with longer test sequences, average interval widths would continue to improve as the methods learn from additional coverage feedback.
